...
6. **Phase 2 deviation — Jackson 3 + `-parameters` changed `@JsonCreator` default mode.** The Boot 4.1.0 build compiles with `-parameters` (added by the Spring Boot Gradle plugin). On Jackson 3, a single-arg `@JsonCreator` constructor with a discoverable parameter name is treated as a *property-based* creator instead of a delegate, so the documented flat-map request contract would deserialize to `null` tags (regression caught by `FeatureCheckRequestTest`, which is the Jackson 3 probe). Fixed by making the mode explicit: `@JsonCreator(mode = JsonCreator.Mode.DELEGATING)` (`FeatureCheckRequest.java:15`). Assertions unchanged.
7. **Phase 2 — Boot 4 test infrastructure.** `@SpringBootTest` no longer auto-configures `WebTestClient`; `@AutoConfigureWebTestClient` moved to `org.springframework.boot.webtestclient.autoconfigure` (module `spring-boot-webtestclient`, added as a test dependency). `WebTestClient`'s `EntityExchangeResult.getStatus()` now returns `HttpStatusCode`.
