...
2. Each emission triggers `GitFeatureFileRepository.loadAll()`, which:
   - calls `GitRepoManager.update()` to pull the tracked branch into a local directory via JGit, offloaded to a bounded-elastic scheduler so the blocking call never holds a reactive pipeline thread; the clone that set up the tracked branch happened earlier in a startup initialization phase (`GitRepoManager` is an `InitializingBean`), and because the branch is resolved and recorded at clone time, changing `features.git.remote.branch` — or the remote's default branch — only takes effect after the local clone is removed and re-initialized;
