...
3. `YamlFileFeatureFactory.read(file)` parses the YAML and assembles a `Feature` (valves, evaluator, active flag). Parsing errors surface as errors in the reactive stream; `FeatureLoader` wraps each read in `.onErrorResume(...)` so a single malformed file is skipped without aborting the tick.
...
5. The tick's `Mono` completes only after the sink has fully processed that terminal signal (eviction included): `FeatureLoader` forwards each signal to the sink first and only then completes the `Mono`. So a caller cannot observe success before the cache is reconciled with the repository — the guarantee behind the scheduler's blocking startup tick, which returns only once reconciliation has finished.
