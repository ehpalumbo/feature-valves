...
3. `LocalFeatureFileRepository` walks `features/<application>/` folders, lists `*.yml`/`*.yaml` files in filename order, and reads each with `AsynchronousFileChannel` through reactive `DataBuffer`s, yielding `FeatureFile` objects carrying the file content and a derived `FeatureId`.
